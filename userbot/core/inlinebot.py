...
DEVLOGO = "https://telegra.ph/file/493268c1f5ebedc967eba.jpg"
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Devuserbot"):
            buttons = [
                (
            ...
            if IALIVE_PIC:
                DEV = [x for x in IALIVE_PIC.split()]
                PIC = list(DEV)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                DEV = [x for x in ALIVE_PIC.split()]
                PIC = list(DEV)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            if DEV_IMG and DEV_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    DEV_IMG,
                    # title="Alive dev",
                    text=query,
...
@devub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@devub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@devub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
