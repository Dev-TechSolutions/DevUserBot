...
async def animator(media, mainevent, textevent):
    # //Hope u dunt kang :/ @Samarth-Dubey
    h = media.file.height
    w = media.file.width
    w, h = (-1, 512) if h > w else (512, -1)
    if not os.path.isdir(Config.TEMP_DIR):
        os.makedirs(Config.TEMP_DIR)
    BadDev = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {BadDev} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(BadDev)
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    dev = await sticcers[c_lick].click("me", hide_via=True)
    if dev:
        await borg.send_file(int(chat_id), dev, reply_to=reply_to_id)
        await dev.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    dev = await sticcers[0].click("me", hide_via=True)
    if dev:
        await bot.send_file(int(chat_id), dev, reply_to=reply_to_id)
        await dev.delete()
