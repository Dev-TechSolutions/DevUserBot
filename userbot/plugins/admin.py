...
async def promote(event):
    ...
    if not user:
        return
    devevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await devevent.edit(NO_PERM)
    await devevent.edit("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    devevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await devevent.edit(NO_PERM)
    await devevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    devevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    devevent = await edit_or_reply(event, "`Unbanning...`")
    try:
...
async def endmute(event):
    ...
    if not user:
        return
    devevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await devevent.edit(NO_PERM + f"\n{e}")
    if reason:
        await devevent.edit(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await devevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "`Searching recent actions .....`")
