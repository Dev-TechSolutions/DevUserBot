...
# baning spmmers plugin for Devuserbot by @sandy1709
...
from . import BOTLOG, BOTLOG_CHATID, devub, edit_or_reply, logging, spamwatch
...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        devadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not devadmin:
            return
        devbanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            devgban = get_gbanuser(user.id)
            if devgban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{devgban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not devbanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await devevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await devevent.edit("`CAS check failed`")
        return
    await devevent.edit(text)
...
async def caschecker(event):
    ...
    try:
        ...
        await devevent.edit("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await devevent.edit("`spamwatch check failed`")
        return
    await devevent.edit(text)
