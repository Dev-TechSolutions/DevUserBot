...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await devub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await devub.upload_file(autophoto_path)
        try:
            await devub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await devub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await devub(
                    functions.photos.DeletePhotosRequest(
                        await devub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await devub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        dev = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(dev, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await devub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await devub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await devub(functions.photos.UploadProfilePhotoRequest(file))
        await _devutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
devub.loop.create_task(autopfp_start())
devub.loop.create_task(autopicloop())
devub.loop.create_task(digitalpicloop())
devub.loop.create_task(bloom_pfploop())
devub.loop.create_task(autoname_loop())
devub.loop.create_task(autobio_loop())
devub.loop.create_task(custompfploop())
