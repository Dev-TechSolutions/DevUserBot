...
@devub.dev_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @Devuserbotot just a testmessage",
    },
)
async def devbroadcast_add(event):
...
async def devbroadcast_add(event):
    ...
    devinput_str = event.pattern_match.group(1)
    if not devinput_str:
        return await edit_delete(
            event,
...
async def devbroadcast_list(event):
    ...
    await edit_or_reply(devevent, finaloutput)
...
async def devbroadcast_send(event):
    ...
    await edit_delete(devevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def devbroadcast_remove(event):
...
async def devbroadcast_delete(event):
    "To delete a category completely."
    devinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(devinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {devinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(devinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {devinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
