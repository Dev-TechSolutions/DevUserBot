...
plugin for dev_Userbot
"""

from telethon.errors.rpcerrorlist import YouBlockedUserError

from . import devub, edit_or_reply, reply_id
...
async def _(event):
    ...
    devevent = await edit_or_reply(event, "```Gathering info...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await devevent.edit("Unblock @cricbuzz_bot & try again")
            return
        if respond.text.startswith("I can't find that"):
            await devevent.edit("sorry i can't find it")
        else:
