...
@devub.dev_cmd(
    ...
    info={
        ...
        "var name": "**[list of vars]**(https://Devuserbot.gitbook.io/Devuserbot/data-vars-setup)",
        "usage": [
    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"Check @dev_alive")
            if not vinfo:
                return await edit_delete(
...
@devub.dev_cmd(
    ...
    info={
        "header": "To customize your Devuserbot.",
        "options": {
...
async def custom_Devuserbot(event):
    "To customize your Devuserbot."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
...
@devub.dev_cmd(
    ...
    info={
        "header": "To delete costomization of your Devuserbot.",
        "options": {
...
async def custom_Devuserbot(event):
    "To delete costomization of your Devuserbot."
    input_str = event.pattern_match.group(1)
    if input_str == "pmpermit":
