...
async def echo(event):
    ...
    devevent = await edit_or_reply(event, "`Adding Echo to user...`")
    user, rank = await get_user_from_event(event, devevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_or_reply(devevent, "Hi")
...
@devub.dev_cmd(incoming=True, edited=False)
