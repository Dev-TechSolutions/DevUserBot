...
@devub.dev_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec dev stringsetup.py",
    },
)
async def _(event):
    ...
    devevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    devuser = await event.client.get_me()
    curruser = devuser.username or "Devuserbot"
    uid = os.geteuid()
    if uid == 0:
...
@devub.dev_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('Devuserbot')",
    },
)
async def _(event):
    ...
    devevent = await edit_or_reply(event, "`Running ...`")
