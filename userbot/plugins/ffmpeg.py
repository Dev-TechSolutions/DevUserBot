...
from ..helpers import _devtools, media_type, progress, reply_id

plugin_category = "utils"


FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "Devuserbot.media.ffmpeg"
)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                devevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
