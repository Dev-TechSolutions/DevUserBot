...
async def lst(event):  # sourcery no-metrics
    ...
    dev = "".join(event.text.split(maxsplit=1)[1:])
    path = dev or os.getcwd()
    if not os.path.exists(path):
        await edit_or_reply(
            event,
            f"there is no such directory or file with the name `{dev}` check again",
        )
        return
    path = Path(dev) if dev else os.getcwd()
    if os.path.isdir(path):
        ...
        for contents in sorted(lists):
            devpath = os.path.join(path, contents)
            if not os.path.isdir(devpath):
                size = os.stat(devpath).st_size
...
async def _(event):
    ...
    try:
        await _devutils.runcmd(f"mkdir {original}")
...
async def _(event):
    ...
    await asyncio.sleep(2)
    try:
        await _devutils.runcmd(f"cp -r {original} {Location}")
