...
async def devgban(event):  # sourcery no-metrics
    ...
    deve = await edit_or_reply(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    if user.id == devub.uid:
        return await edit_delete(deve, "`why would I ban myself`")
    ...
    samarth = len(san)
    if samarth == 0:
        return await edit_delete(deve, "`you are not admin of atleast one group` ")
    await deve.edit(
        f"`initiating gban of the `[user](tg://user?id={user.id}) `in {len(san)} groups`"
    )
    for i in range(samarth):
        try:
    ...
    if reason:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {devtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {devtaken} seconds`!!"
...
@devub.dev_cmd(
    ...
    command=("ungban", plugin_devegory),
    info={
...
async def devgban(event):
    ...
    deve = await edit_or_reply(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.devungban(user.id)
    else:
        return await edit_delete(
            deve, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = await admin_groups(event.client)
    count = 0
    samarth = len(san)
    if samarth == 0:
        return await edit_delete(deve, "`you are not even admin of atleast one group `")
    await deve.edit(
    ...
    if reason:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {devtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {devtaken} seconds`!!"
...
@devub.dev_cmd(
    pattern="listgban$",
    command=("listgban", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("ungmute", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="gkick(?:\s|$)([\s\S]*)",
    command=("gkick", plugin_devegory),
    info={
        "header": "kicks the person in all groups where you are admin.",
        "usage": "{tr}gkick <username/reply/userid> <reason (optional)>",
    },
)
async def devgkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    deve = await edit_or_reply(event, "`gkicking.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    if user.id == devub.uid:
        return await edit_delete(deve, "`why would I kick myself`")
    san = await admin_groups(event.client)
    count = 0
    samarth = len(san)
    if samarth == 0:
