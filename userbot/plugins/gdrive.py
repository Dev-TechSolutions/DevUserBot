...
plugin_devegory = "misc"

# Devuserbot Google Drive managers  ported from Projectbish and added extra things by @Samarth-Dubey
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using Devuserbot.",
        "mimeType": mimeType,
...
async def generate_credentials(gdrive):
    ...
    msg = await gdrive.respond(
        "`Go to your Private log group to authentideve token...`"
    )
...
@devub.dev_cmd(
    pattern="greset",
    command=("greset", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="gabort$",
    command=("gabort", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="gclear$",
    command=("gclear", plugin_devegory),
    info={
...
async def g_download(event):
    ...
    file_name, devprocess = await gdrive_download(event, devevent, service, drive_link)
    if devprocess is not None:
        return await edit_delete(devevent, file_name)
    thumb = thumb_image_path if os.path.exists(thumb_image_path) else None
    if not cmd:
        await devevent.edit("**File Downloaded.\nLocation : **`" + str(file_name) + "`")
    else:
        c_time = time.time()
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, devevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
@devub.dev_cmd(
    pattern="gshare ([\s\S]*)",
    command=("gshare", plugin_devegory),
    info={
...
async def gshare(event):
    ...
    devevent = await edit_or_reply(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, devevent, input_str)
