...
ported to telethon by @Samarth-Dubey and @samarth1709
"""
...
from ..helpers.utils import _devtools, _devutils, reply_id
...
@devub.dev_cmd(
    ...
    command=("glitch", plugin_devegory),
    info={
...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    devinput = int(devinput) if devinput else 2
    glitch_file = await _devtools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, devinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=devid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, devinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
        ...
        samarth = await event.client.send_file(event.chat_id, glitched, reply_to=devid)
