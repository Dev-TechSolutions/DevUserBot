...
async def ban_user(chat_id, i, rights):
    try:
        await devub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
@devub.dev_cmd(
    pattern="kickme$",
    command=("kickme", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="kickall$",
    command=("kickall", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="banall$",
    command=("banall", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="unbanall$",
    command=("unbanall", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("zombies", plugin_devegory),
    info={
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if flag != " -r":
            ...
            devadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not devadmin:
