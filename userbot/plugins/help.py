...
async def cmdinfo(input_str, event, plugin=False):
    ...
    if plugin is not None:
        ...
        devegory = getkey(plugin)
        if devegory is not None:
            outstr += f"**devegory :** `{devegory}`\n\n"
    outstr += f"**✘  Intro :**\n{about[0]}"
    return outstr
...
async def grpinfo():
    outstr = "**Plugins in Devuserbot are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    devegory = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for dev in devegory:
        plugins = GRP_INFO[dev]
        outstr += f"**{hemojis[dev]} {dev.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Devuserbot are :**\n\n"
    devegory = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for dev in devegory:
        plugins = GRP_INFO[dev]
        outstr += f"**{hemojis[dev]} {dev.title()} ** - {len(plugins)}\n\n"
...
@devub.dev_cmd(
    ...
    command=("help", plugin_devegory),
    info={
        "header": "To get guide for Devuserbot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of Devuserbot are :"
    )


@devub.dev_cmd(
    pattern="s ([\s\S]*)",
    command=("s", plugin_devegory),
    info={
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in Devuserbot"
    await edit_or_reply(event, out)


@devub.dev_cmd(
    pattern="dc$",
    command=("dc", plugin_devegory),
    info={
