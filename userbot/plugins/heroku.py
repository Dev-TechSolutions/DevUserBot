# Heroku manager for your Devuserbot
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        dev = await edit_or_reply(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            if variable in heroku_var:
                return await dev.edit(
        ...
        dev = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await dev.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await dev.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await dev.edit(
        ...
        dev = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await dev.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await dev.edit(f"`{variable}`**  does not exist**")

        await dev.edit(f"`{variable}`  **successfully deleted**")
        del heroku_var[variable]


@devub.dev_cmd(
    pattern="usage$",
    command=("usage", plugin_devegory),
    info={
...
def prettyjson(obj, indent=2, maxlinelength=80):
    """Renders JSON content with indentation and line splits/condevenations to fit maxlinelength.
