...
@devub.dev_cmd(
    ...
    command=("imirror", plugin_devegory),
    info={
...
async def imirror(event):  # sourcery no-metrics
    ...
    devevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "Devuserbot.webp"
        f_format = "webp"
    else:
        filename = "Devuserbot.jpg"
        f_format = "jpeg"
    try:
...
@devub.dev_cmd(
    pattern="irotate(?: |$)(\d+)$",
    command=("irotate", plugin_devegory),
    info={
...
async def irotate(event):
    ...
    args = event.pattern_match.group(1)
    devevent = await edit_or_reply(event, "__Rotating the replied media...__")
    imag = await _devtools.media_to_pic(devevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
@devub.dev_cmd(
    ...
    command=("iresize", plugin_devegory),
    info={
...
async def iresize(event):
    ...
    devevent = await edit_or_reply(event, "__Resizeing the replied media...__")
    imag = await _devtools.media_to_pic(devevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
    ...
    if len(args) == 1:
        try:
            ...
            return await edit_delete(devevent, "**Error:**\n__Invalid dimension.__")
    else:
        try:
            nw = int(args[0])
        except ValueError:
    ...
    try:
        ...
        return await edit_delete(devevent, f"**Error:** __While resizing.\n{e}__")
    await event.delete()
    img = io.BytesIO()
    img.name = "Devuserbot.png"
    image.save(img, "PNG")
...
@devub.dev_cmd(
    pattern="square$",
    command=("square", plugin_devegory),
    info={
...
async def square_cmd(event):
    ...
    devevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await _devtools.media_to_pic(devevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
...
@devub.dev_cmd(
    pattern="dotify(?: |$)(\d+)?$",
    command=("dotify", plugin_devegory),
    info={
...
async def pic_gifcmd(event):
    ...
    if args:
        ...
        pix = 100
    devevent = await edit_or_reply(event, "__🎞Dotifying image...__")
    imag = await _devtools.media_to_pic(devevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
