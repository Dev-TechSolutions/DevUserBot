...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = await devub(GetFullUserRequest(LASTFM_.USER_ID))
            ...
            if (
                ...
                try:
                    ...
                    await devub(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await devub(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await devub(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await devub.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await devub.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
...
@devub.dev_cmd(
    pattern="lastfm$",
    command=("lastfm", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("lastbio", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("lastlog", plugin_devegory),
    info={
