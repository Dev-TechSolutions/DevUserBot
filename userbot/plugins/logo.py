...
async def bad(event):
    ...
    for i, each in enumerate(links, start=1):
        dev = os.path.splitext(each.text)[0]
        bg_name.append(dev)
        lbg_list += f"**{i}.**  `{dev}`\n"
    ...
    if cmd == "c":
        ...
        if not input_str and event.reply_to_msg_id and reply_message.media:
            if not os.path.isdir("./temp"):
                os.mkdir("./temp")
            output = await _devtools.media_to_pic(event, reply_message)
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        dev = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(dev, input_str)
