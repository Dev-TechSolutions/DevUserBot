...
@devub.dev_cmd(
    ...
    command=("lyrics", plugin_devegory),
    info={
...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        try:
            ...
            songs = None
        if songs is None:
            return await devevent.edit(f"Song **{artist} - {song}** not found!")
        result = f"**Search query**: \n`{artist} - {song}`\n\n```{songs.lyrics}```"
    else:
        devevent = await edit_or_reply(event, f"`Searching lyrics for {query}...`")
        response = genius.search_songs(query)
        msg = f"**The songs found for the given query:** `{query}`\n\n"
        if len(response["hits"]) == 0:
            return await edit_or_reply(
                devevent, f"**I can't find lyrics for the given query: **`{query}`"
            )
    ...
    await edit_or_reply(devevent, result)
