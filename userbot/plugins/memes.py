...
# Devuserbot module for having some fun with people.
import asyncio
import random
import re
...
from ..helpers import devmemes
from ..helpers.utils import _devutils, parse_pre
...
@devub.dev_cmd(
    pattern="(\w+)say ([\s\S]*)",
    command=("cowsay", plugin_devegory),
    info={
        ...
        "examples": [
            "{tr}squirrelsay Devuserbot",
            "{tr}milksay Devuserbot",
            "{tr}ghostbustersghostbusterssay Devuserbot",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await devmemes.slap(replied_user, event, mention)
    try:
...
@devub.dev_cmd(
    ...
    command=("decide", plugin_devegory),
    info={
...
async def decide(event):
    ...
    await event.delete()
    samarth = await event.client.send_message(
        event.chat_id, str(r["answer"]).upper(), reply_to=message_id, file=r["image"]
    )
    await _devutils.unsavegif(event, samarth)


@devub.dev_cmd(
    pattern="shout(?:\s|$)([\s\S]*)",
    command=("shout", plugin_devegory),
    info={
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", " " + random.choice(devmemes.UWUS), reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += " " + random.choice(devmemes.UWUS)
    await edit_or_reply(owo, reply_text)


@devub.dev_cmd(
    pattern="clap(?:\s|$)([\s\S]*)",
    command=("clap", plugin_devegory),
    info={
...
async def gbun(event):
    ...
    devevent = await edit_or_reply(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await devevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await devevent.edit(mention)
