# Made by @Samarth-Dubey and @samarth1709
# memify plugin for Devuserbot
import asyncio
import base64
import io
import os
...
from ..helpers import asciiart, dev_meeme, dev_meme, media_type
...
from ..helpers.utils import _devtools, reply_id
...
@devub.dev_cmd(
    pattern="pframe(f|-f)?$",
    command=("pframe", plugin_devegory),
    info={
...
async def maccmd(event):  # sourcery no-metrics
    ...
    devevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await _devtools.media_to_pic(devevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
        ...
        return await edit_delete(devevent, f"**Error in identifying image:**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    meme = os.path.join("./temp", "devmeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "userbot/helpers/styles/impact.ttf"
    else:
        CNG_FONTS = gvarstatus("CNG_FONTS")
    if max(len(top), len(bottom)) < 21:
        await dev_meme(CNG_FONTS, top, bottom, meme_file, meme)
    else:
        await dev_meeme(top, bottom, CNG_FONTS, meme_file, meme)
    if cmd != "mmf":
        meme = convert_tosticker(meme)
    await event.client.send_file(
        event.chat_id, meme, reply_to=devid, force_document=False
    )
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await devevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not devinput else devinput
    asciiart(meme_file, 0.3, 1.9, outputfile, color1, color2, bgcolor)
    await event.client.send_file(
        event.chat_id, outputfile, reply_to=devid, force_document=False
    )
    await output[0].delete()
...
@devub.dev_cmd(
    pattern="invert$",
    command=("invert", plugin_devegory),
    info={
...
async def memes(event):
    ...
    await event.client.send_file(
        event.chat_id, outputfile, force_document=False, reply_to=devid
    )
    await output[0].delete()
    for files in (outputfile, meme_file):
        if files and os.path.exists(files):
...
@devub.dev_cmd(
    pattern="solarize$",
    command=("solarize", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="mirror$",
    command=("mirror", plugin_devegory),
    info={
...
async def memes(event):
    ...
    devinput = event.pattern_match.group(1)
    devinput = 50 if not devinput else int(devinput)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
