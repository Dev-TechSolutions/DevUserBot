...
@devub.dev_cmd(
    pattern="detect$",
    command=("detect", plugin_devegory),
    info={
...
async def detect(event):
    ...
    devevent = await edit_or_reply(event, "`Downloading the file to check...`")
    media = await event.client.download_media(reply)
    if not media.endswith(("png", "jpg", "webp")):
        return await edit_delete(
            event, "`Reply to any image or non animated sticker !`", 5
        )
    devevent = await edit_or_reply(event, "`Detecting NSFW limit...`")
    r = requests.post(
    ...
    os.remove(media)
    if "status" in r.json():
        return await edit_delete(devevent, r.json()["status"])
