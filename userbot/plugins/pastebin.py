...
async def paste_img(event):
    ...
    d_file_name = None
    devevent = await edit_or_reply(event, "`Pasting the text on image`")
    ...
    try:
        ...
        await edit_delete(devevent, f"**Error:**\n`{e}`", time=10)


@devub.dev_cmd(
    pattern="(d|p|s|n)?(paste|neko)(?:\s|$)([\S\s]*)",
    command=("paste", plugin_devegory),
    info={
...
async def paste_bin(event):
    ...
    devevent = await edit_or_reply(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await devevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(devevent, f"**Error while pasting text:**\n`{e}`")


@devub.dev_cmd(
    command=("neko", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("getpaste", plugin_devegory),
    info={
...
async def get_dogbin_content(event):
    ...
    devevent = await edit_or_reply(event, "`Getting Contents of pastebin.....`")
    rawurl = None
    if "raw" in url:
        rawurl = url
    if rawurl is None:
        ...
        if "pasty" in url:
            ...
            rawurl = f"http://devbin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await devevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await edit_or_reply(devevent, reply_text)


@devub.dev_cmd(
    pattern="paster(?:\s|$)([\s\S]*)",
    command=("paster", plugin_devegory),
    info={
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(devevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await devevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await devevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await devevent.edit(result, link_preview=True)
