...
You have {warns}/{totalwarns} warns until you get blocked by the Devuserbot.
...
Don't spam my inbox. say reason and wait until my response.__"""
    ...
    try:
        if gvarstatus("pmmenu") is None:
            ...
            if PM_PIC:
                dev = [x for x in PM_PIC.split()]
                PIC = list(dev)
                dev_IMG = random.choice(PIC)
            else:
                dev_IMG = None
...
@devub.dev_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@devub.dev_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@devub.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@devub.dev_cmd(
    ...
    command=("pmguard", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("pmmenu", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("approve", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("disapprove", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("unblock", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("listapproved", plugin_devegory),
    info={
