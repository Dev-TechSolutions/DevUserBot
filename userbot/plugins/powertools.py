...
@devub.dev_cmd(
    pattern="restart$",
    command=("restart", plugin_devegory),
    info={
...
async def _(event):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [samarth.chat_id, samarth.id])
    except Exception as e:
        LOGS.error(e)
    try:
        await devub.disconnect()
...
@devub.dev_cmd(
    pattern="shutdown$",
    command=("shutdown", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("notify", plugin_devegory),
    info={
