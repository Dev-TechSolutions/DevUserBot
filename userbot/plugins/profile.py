...
async def _(event):
    ...
    try:
        ...
        await devevent.edit(str(e))
    else:
        if photo:
            await devevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await devevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                devpic = None
                devvideo = await event.client.upload_file(photo)
            else:
                devpic = await event.client.upload_file(photo)
                devvideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=devpic, video=devvideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                ...
                await edit_or_reply(
                    devevent, "`My profile picture was successfully changed`"
                )
    try:
        os.remove(photo)
    except Exception as e:
...
@devub.dev_cmd(
    ...
    command=("pusername", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="delpfp ?([\s\S]*)",
    command=("delpfp", plugin_devegory),
    info={
...
@devub.dev_cmd(
    pattern="myusernames$",
    command=("myusernames", plugin_devegory),
    info={
