...
@devub.dev_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic Devuserbot.", "{tr}qpic -b Devuserbot."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "Devuserbot.Webp"
        img.save(output, "webp")
    else:
        output.name = "Devuserbot.png"
        img.save(output, "PNG")
    output.seek(0)
...
@devub.dev_cmd(
    pattern="q(?:\s|$)([\s\S]*)",
    command=("q", plugin_devegory),
    info={
        "header": "Makes your message as sticker quote.",
        "usage": "{tr}q",
    },
)
async def stickerchat(devquotes):
    "Makes your message as sticker quote"
    reply = await devquotes.get_reply_message()
    if not reply:
        return await edit_or_reply(
            devquotes, "`I cant quote the message . reply to a message`"
        )
    fetchmsg = reply.message
    repliedreply = None
    mediatype = media_type(reply)
    if mediatype and mediatype in ["Photo", "Round Video", "Gif"]:
        return await edit_or_reply(devquotes, "`Replied message is not supported now`")
    devevent = await edit_or_reply(devquotes, "`Making quote...`")
    user = (
        await devquotes.client.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, devmsg = await process(fetchmsg, user, devquotes.client, reply, repliedreply)
    if not res:
        return
    outfi = os.path.join("./temp", "sticker.png")
    devmsg.save(outfi)
    endfi = convert_tosticker(outfi)
    await devquotes.client.send_file(devquotes.chat_id, endfi, reply_to=reply)
    await devevent.delete()
    os.remove(endfi)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    devevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    devevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await devevent.edit("```Please unblock me (@QuotLyBot) u Nigga```")
