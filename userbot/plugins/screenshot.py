...
@devub.dev_cmd(
    pattern="ss ([\s\S]*)",
    command=("ss", plugin_devegory),
    info={
        "header": "To Take a screenshot of a website.",
        "usage": "{tr}ss <link>",
        "examples": "{tr}ss https://github.com/Tgdevub/Devuserbot",
    },
)
async def _(event):
    ...
    try:
        ...
        chrome_options.add_argument("--ignore-certifideve-errors")
        ...
        if not devurl:
            ...
            devurl = url(inputstr)
        if not devurl:
            return await devevent.edit("`The given input is not supported url`")
        driver.get(inputstr)
        await devevent.edit("`Calculating Page Dimensions`")
        ...
        await devevent.edit("`Stoppping Chrome Bin`")
        driver.close()
...
async def _(event):
    ...
    if "image" in contentType:
        ...
        await devevent.edit(f"`{response_api.text}`")
