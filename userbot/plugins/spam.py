...
async def spam_function(event, samarth, dev, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(dev[0])
    if len(dev) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await samarth.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and samarth.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            samarth = await event.client.send_file(
                event.chat_id, samarth, caption=samarth.text
            )
        ...
        if BOTLOG:
            ...
            samarth = await event.client.send_file(BOTLOG_CHATID, samarth)
            await _devutils.unsavegif(event, samarth)
        return
    elif event.reply_to_msg_id and samarth.text:
        spam_message = samarth.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    samarth = await event.get_reply_message()
    ...
    await spam_function(event, samarth, dev, sleeptimem, sleeptimet)


@devub.dev_cmd(
    pattern="spspam$",
    command=("spspam", plugin_devegory),
    info={
...
@devub.dev_cmd(
    ...
    command=("delayspam", plugin_devegory),
    info={
