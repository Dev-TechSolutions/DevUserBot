...
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@devfonts", font_file_name)
    ...
    image_stream.name = "devuserbot.webp"
...
async def quby(event):
    ...
    if cmd == "b":
        dev = convert_tosticker(file_name)
        await event.client.send_file(
            event.chat_id, dev, reply_to=reply_to_id, force_document=False
        )
    else:
