...
# modified and developed by @Samarth-Dubey , @jisan7509
...
def verify_cond(devarray, text):
    return any(i in text for i in devarray)


def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"DevUserbot_{userid}_{pack}_anim"
    if is_video:
        return f"DevUserbot_{userid}_{pack}_vid"
    return f"DevUserbot_{userid}_{pack}"
...
async def delpack(devevent, conv, cmd, args, packname):
    try:
        await conv.send_message(cmd)
    except YouBlockedUserError:
        await devevent.edit("You have blocked the @stickers bot. unblock it and try.")
        return None, None
...
async def add_to_pack(
    ...
    while ("50" in x.text) or ("120" in x.text):
        ...
        await devevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await devevent.edit("check `.info stickers`")
...
async def pussycat(args):
    "To kang a sticker."  # scam :('  Dom't kamg :/@Samarth-Dubey
    message = await args.get_reply_message()
    user = await args.client.get_me()
    userid = user.id
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"Dev_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await devevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
