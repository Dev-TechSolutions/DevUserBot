# speech to text module for Devuserbot by uniborg (@spechide)
import os
...
async def _(event):
    ...
    devevent = await edit_or_reply(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await devevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await devevent.edit(r["error"])
    ...
    await devevent.edit(string_to_show)
