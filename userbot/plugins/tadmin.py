...
async def tmuter(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "`muting....`")
    user, reason = await get_user_from_event(event, devevent)
    if not user:
        return
    if not reason:
        return await devevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    devtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(devevent, devtime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await devevent.edit("Sorry, I can't mute myself")
    try:
        await devevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "`banning....`")
    user, reason = await get_user_from_event(event, devevent)
    if not user:
        return
    if not reason:
        return await devevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    devtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await devevent.edit("Sorry, I can't ban myself")
    await devevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
