...
from ..helpers.utils import _devutils, _format, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/Tgdevub/Devuserbot-Resources/master/Resources/Data/currency.py"
        )
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if response1.text:
                ...
                return await devevent.edit(response1.text, parse_mode=_format.parse_pre)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        ...
        if not input_str:
            return await edit_or_reply(devevent, response4.text)
