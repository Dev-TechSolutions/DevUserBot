# credits to @Samarth-Dubey and @sandy1709
...
async def devbot(event):
    ...
    await event.client.send_file(event.chat_id, dev, reply_to=devid)
...
async def devbot(event):
    ...
    try:
        ...
        return await output[0].edit(f"**Error: **\n`{exc}`")
    dev = f"https://telegra.ph{response[0]}"
    dev = await trap(text1, text2, dev)
    await output[0].delete()
    os.remove(download_location)
    await event.client.send_file(event.chat_id, dev, reply_to=devid)
...
async def devbot(event):
    ...
    try:
        ...
        return await output[0].edit(f"**Error: **\n`{exc}`")
    dev = f"https://telegra.ph{response[0]}"
    dev = await phcomment(dev, text, username)
    await output[0].delete()
    os.remove(download_location)
    await event.client.send_file(event.chat_id, dev, reply_to=devid)
