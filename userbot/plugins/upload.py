...
async def devlst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        devpath = os.path.join(path, file)
...
async def upload(path, event, udir_event, devflag=None):  # sourcery no-metrics
    devflag = devflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            devpath = os.path.join(path, file)
            await upload(Path(devpath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, devflag=flag)
        end = datetime.now()
