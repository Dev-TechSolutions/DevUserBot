...
async def download_audio(event):
    ...
    devevent = await edit_or_reply(event, "`Preparing to download...`")
    reply_to_id = await reply_id(event)
    try:
        vid_data = YoutubeDL({"no-playlist": True}).extract_info(url, download=False)
    except ExtractorError:
        vid_data = {"title": url, "uploader": "Devuserbot", "formats": []}
    startTime = time()
...
async def download_video(event):
    ...
    devthumb = pathlib.Path(f"{ytdl_data['title']}.jpg".replace("|", "_"))
    if not os.path.exists(devthumb):
        devthumb = pathlib.Path(f"{ytdl_data['title']}.webp".replace("|", "_"))
    if not os.path.exists(devthumb):
        devthumb = None
    await devevent.edit(
        f"`Preparing to upload video:`\
    ...
    uploaded = await event.client.fast_upload_file(
        file=ul,
        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
            progress(d, t, devevent, c_time, "upload", file_name=f)
        ),
    )
    ul.close()
    media = types.InputMediaUploadedDocument(
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        devevent = await edit_or_reply(event, "**Downloading.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await devevent.edit("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await devevent.delete()
        dev = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()
