...
class Gdrive(BASE):
    __tablename__ = "DevGdrive"
    dev = Column(String(50), primary_key=True)
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.dev == str(folder_id))
...
def rmparent_id(folder_id):
    if note := SESSION.query(Gdrive).filter(Gdrive.dev == folder_id):
        note.delete()
        SESSION.commit()
