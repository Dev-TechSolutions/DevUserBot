...
class Dev_GlobalCollection(BASE):
    __tablename__ = "Dev_globalcollection"
    ...
    def __repr__(self):
        return "<Dev Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Dev_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Dev_GlobalCollection.__table__.create(checkfirst=True)

Dev_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with Dev_GLOBALCOLLECTION:
        keyword_items = Dev_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with Dev_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Dev_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with Dev_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Dev_GlobalCollection.keywoard)
            .filter(Dev_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Dev_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Dev_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Dev_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Dev_GlobalCollection).all()
        for x in all_groups:
