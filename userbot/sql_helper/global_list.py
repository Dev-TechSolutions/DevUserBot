...
class DevGloballist(BASE):
    __tablename__ = "DevGlobal_list"
    ...
    def __repr__(self):
        return "<Dev global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, DevGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


DevGloballist.__table__.create(checkfirst=True)

DevGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with DevGLOBALLIST_INSERTION_LOCK:
        broadcast_group = DevGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with DevGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(DevGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with DevGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(DevGloballist.keywoard)
            .filter(DevGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(DevGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(DevGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(DevGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(DevGloballist).all()
        for x in all_groups:
